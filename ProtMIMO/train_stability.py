...
def get_stability_metrics(targets, preds, preds_by_input, num_inputs, loss_fn, test_df=test_df, ensemble=False):
    metrics = {}
    test_loss = loss_fn(torch.tensor(targets), torch.tensor(preds)).item()
    metrics['test_loss'] = test_loss
    metrics.update(compute_stability_scalar_metrics(targets=targets, preds=preds, prefix=None, test_df=test_df))
    for i in range(num_inputs):
        metrics[f'model_{i}_test_loss'] = loss_fn(torch.tensor(targets), torch.tensor(preds_by_input[f'model_{i}'])).item()
        metrics.update(compute_stability_scalar_metrics(targets=targets, preds=preds, prefix=f'model_{i}', test_df=test_df))
    ...
    create_plot(targets=targets, preds=preds, title=title, path=path, feature_name='Stability')
    ...
    for i in range(num_inputs):
        ...
        create_plot(targets, preds_by_input[f'model_{i}'], title=title, path=path, feature_name='Stability')
...
def train_stability_mimo_model(num_inputs=3, bs=32, lr=0.001, num_epochs=100, patience=10):
    ...
    best_model, test_metrics = train_and_evaluate(model=model, data=data, metrics_fn=get_stability_metrics, num_inputs=num_inputs, lr=lr, num_epochs=num_epochs, patience=patience)
    pprint(test_metrics)
...
def train_stability_ensemble_models(num_inputs=3, bs=32, lr=0.001, num_epochs=100, patience=10):
    ...
    best_models, test_metrics = train_and_evaluate_ensemble(models=models, data=data, metrics_fn=get_stability_metrics, lr=lr, num_epochs=num_epochs, patience=patience)
    pprint(test_metrics)
