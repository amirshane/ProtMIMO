...
def batch_seq_encode(batch_seqs, alphabet, max_len=None):
    ...
    batch_encoded_seqs = np.array(
        [multi_seq_encode(seqs, alphabet, max_len) for seqs in batch_seqs]
    )
...
class ProtMIMOOracle(nn.Module):
    ...
    def __init__(
        ...
        self.mhl = MultiHeadLinear(
            self.multi_head_input_dim, num_outputs, self.num_inputs
        )
    ...
    def _init_conv_layers(self):
        ...
        for i in range(num_conv_blocks):
            ...
            conv_output_size = (
                floor((conv_output_size - kernel_sizes[i]) / 1) + 1
            )  # division by 1 is stride size
            if pooling_dims[i] > 0:
